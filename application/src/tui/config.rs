...
pub const PROJECT_NAME: LazyLock<String> =
    LazyLock::new(|| env!("CARGO_CRATE_NAME").to_uppercase().to_string());
const DATA_FOLDER: LazyLock<Option<PathBuf>> = LazyLock::new(|| {
    std::env::var(format!("{}_DATA", LazyLock::force(&PROJECT_NAME)))
        .ok()
        .map(PathBuf::from)
});
const CONFIG_FOLDER: LazyLock<Option<PathBuf>> = LazyLock::new(|| {
    std::env::var(format!("{}_CONFIG", LazyLock::force(&PROJECT_NAME)))
        .ok()
        .map(PathBuf::from)
});
...
fn project_directory() -> Option<ProjectDirs> {
    ProjectDirs::from("com", "knowls", env!("CARGO_PKG_NAME"))
}
