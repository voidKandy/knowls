...
impl DatabaseConfig {
    /// helper function for displaying config in tui
    pub fn tui_display(&self) -> Vec<Line> {
        vec![
            Line::from(format!("Database: {}", self.database)),
            Line::from(format!("Namespace: {}", self.namespace)),
            Line::from(format!("User: {}", self.user)),
            Line::from(format!("Pass: {}", self.pass)),
            Line::from(format!("Host: {}", self.host)),
            Line::from(format!("Port: {}", self.port)),
            Line::from(format!("Protocol: {:#?}", self.protocol)),
        ]
    }
}
...
// We need this static arr to return for deserialize errors
