...
use super::socket::{CLIENT_SOCKET_ADDR, SERVER_SOCKET_ADDR};
use lsp_types::{
...
pub async fn start_lsp() -> anyhow::Result<()> {
    ...
    if Path::new(CLIENT_SOCKET_ADDR).exists() {
        std::fs::remove_file(CLIENT_SOCKET_ADDR).unwrap();
    }
    let unix_listener = UnixListener::bind(CLIENT_SOCKET_ADDR).unwrap();
